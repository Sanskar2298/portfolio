      ...
      <div className="space-y-6">
        {projects.map((p, i) => (
          <motion.div
            ...
            className="group relative bg-surface border border-border rounded-2xl p-8 overflow-hidden card-hover"
          >
            {/* Background number */}
            <div
              className="absolute -right-4 -top-6 font-display font-bold text-[8rem] leading-none opacity-[0.04] select-none pointer-events-none"
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-start gap-6">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-xs font-display tracking-widest text-text-dim uppercase">
                    {p.number}
                  </span>
                  <span className="w-4 h-px bg-border" />
                  <span className="text-xs text-text-dim">{p.tagline}</span>
                </div>
                <h3 className="font-display font-bold text-2xl text-text-primary mb-3 group-hover:text-accent-soft transition-colors">
                  {p.title}
                </h3>
                <p className="text-text-muted text-sm leading-relaxed mb-5 max-w-xl">
                  {p.description}
                </p>
                <div className="flex flex-wrap gap-2">
                  {p.stack.map((s) => (
                    <span
                      key={s}
                      className="text-xs px-2.5 py-1 rounded-md bg-surface-2 border border-border text-text-muted"
                    >
                      {s}
                    </span>
                  ))}
              ...
              <div className="flex md:flex-col gap-3 flex-shrink-0">
                <a
                  ...
                  className="inline-flex items-center gap-1.5 text-xs text-text-muted hover:text-text-primary border border-border hover:border-accent/30 rounded-lg px-3 py-2 transition-all duration-200"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-xs text-accent hover:text-accent-soft border border-accent/30 hover:border-accent/60 rounded-lg px-3 py-2 transition-all duration-200"
                >
